...
def test_wrap_builtin_error_handling(interpreter):
    ...
    mock_builtin_fn_with_stack = Mock(side_effect=ArslaRuntimeError("Another Error", [], "MOCK"))
...
def test_wrap_control_error_handling(interpreter):
    ...
    mock_control_fn_with_stack = Mock(side_effect=ArslaRuntimeError("More Control Error", [], "CTL"))
...
def test_while_loop_basic(interpreter):
    interpreter.stack = [2, [MockToken("NUMBER", 1), MockToken("SYMBOL", "-"), MockToken("SYMBOL", "D")]]
