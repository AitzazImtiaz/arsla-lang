...
"""
    parser = argparse.ArgumentParser(prog='arsla', description='Arsla Code Golf Language Runtime')
  ...
  """
        ...
        return path
    run_parser.add_argument('file', type=ah_file, help='Arsla source file to execute (must end in .aw)')
...
def open_docs(build: bool):
  ...
  """
    if build:
        subprocess.run([sys.executable, '-m', 'mkdocs', 'build', '-d', '_build'], check=True)
